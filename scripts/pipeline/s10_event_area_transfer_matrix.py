...
def run(cfg: dict, data: dict) -> dict:
    ...
    for _, ev in events_sorted.iterrows():
        ...
        accounting_summary_rows.append({
            ...
            "predecessor_names": " | ".join(ck_to_name.get(ck, ck) for ck in pred_cks),
            "predecessor_cks": " | ".join(pred_cks),
            "districts_after": len(succ_cks),
            "successor_names": " | ".join(ck_to_name.get(ck, ck) for ck in succ_cks),
            "successor_cks": " | ".join(succ_cks),
            "pre_vintage": pre,
            "post_vintage": post,
